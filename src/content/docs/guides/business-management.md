...
> **[③-1 インタビューを実施する](/guides/interview-management/) で生成された業務が、ここ（業務一覧）に集まります。** 内容を確認・整理し、このページを起点に「ワークフロー生成」「クイックウィン分析」へ分岐します。
...
各業務には「タスク量スコア」「自動化適正スコア」「関連人数スコア」の3つのスコアが設定されます。これらは手動で入力するものではなく、**クイックウィン分析時にAIが自動で算出・設定**します。
...
- **クイックウィン分析**：どの業務から着手すべきかをバブルチャートで可視化します。→ 詳細は [③-4 クイックウィンを分析する](/guides/quick-win-analysis/)
